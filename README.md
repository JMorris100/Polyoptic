...
**The placeholder data is invented.** Every figure shipped in `explore.html`
is a plausible-looking shape with no relationship to reality. It exists to
exercise the layout. Until `ingest.py` has run successfully, nothing on the
site is a statistic.

**The identifiers in `series.yaml` are the right shape but unverified.** The
CDIDs, dataset ids and EES hashes need checking against the live catalogues —
the EES entries are explicit `REPLACE_WITH_…` placeholders. Start with
`https://api.beta.ons.gov.uk/v1/datasets` and the EES data catalogue, then run
`--dry-run` and fix what fails. Expect the first pass to take an afternoon;
this is the unglamorous part and there's no way round it.
...
- Deflator source is a stub; the GDP deflator is published as a spreadsheet
  and needs a proper parser.
- No tests.
